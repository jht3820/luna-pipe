...
public class EgovFileScrty {
    ...
    /**
     * SHA-512 비밀번호 암호화
     *  산업안전보건공단 암호화 적용 (입력받은 사용자 비밀번호만 SHA-512로 암호화)
     * 
     * @param password 암호화될 패스워드
     * @return
     * @throws Exception
     */
    public static String encryptPasswordSha512(String password) throws Exception {

		if (password == null) {
		    return "";
		}
		
		String rtnEncrypt = null;
		...
		try {
			...
			rtnEncrypt = String.format("%0128x", new BigInteger(1, digest.digest()));
			
		} catch (Exception e) {
			LOGGER.error("encryptPasswordSha512() User password encryption error..........", e);
		}
