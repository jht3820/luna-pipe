...
var dplSignGrid_config = {
      ...
      columns: [
                ...
                {key: "prjNm", label: "프로젝트 명", width: 150, align: "center"},
         		...
         		{key: "dplNm", label: "배포계획명", width: 269, align: "center"},
      ...
      body: {
          ...
          onClick: function () {
          	...
          	// 더블클릭 시 상세보기
          	var item = this.item;
          	var data = {"prjId":item.prjId,"dplId" : item.dplId, "callView":"dsh4000"};
      ...
      page: {
          ...
          onChange: function () {
             fnInGridListSetting(this.page.selectPage,"&signUsrId=${sessionScope.loginVO.usrId}&prjGrpId="+pagePrjGrpId,gridObj["dplSign"],"/dpl/dpl2000/dpl2100/selectDpl2100AjaxView.do",false);
          }
      } 
  };
...
var signGrid_config = {
	...
	body: {
		...
		columnHeight: 30,
		onDBLClick:function(){
			var popupTargetFlag = false;
			var reqProType = this.item.reqProType;
			
			//권한 체크해서 사용자인경우 상세보기로 전환
			if(usrTyp == "01"){
				popupTargetFlag = true;
			}
			//요구사항이 완료된상태면 상세보기로 전환
			else if(reqProType == "04" || reqProType == "05" || reqProType == "06"){
				popupTargetFlag = true;
			}
			
			if(popupTargetFlag){
				var data = {"mode":"newReq","prjId":this.item.prjId, "reqId": this.item.reqId, "reqProType":"02"}; 
				gfnLayerPopupOpen("/req/req4000/req4100/selectReq4104View.do", data, '1470', '900','scroll');
			}else{
				var data = {"prjGrpId": this.item.prjGrpId,"prjId":this.item.prjId, "reqId": this.item.reqId}; 
	...
	contextMenu: {
		...
		onClick: function (item, param) {
			...
			if(item.type == "reply"){
           	...
           	//상세 정보
				...
				var data = {"mode":"newReq","prjId":param.item.prjId, "reqId": param.item.reqId, "reqProType":"02"}; 
				gfnLayerPopupOpen("/req/req4000/req4100/selectReq4104View.do", data, '1470', '900','scroll');
      ...
      page: {
          ...
          lastIcon: '<i class="fa fa-step-forward" aria-hidden="true"></i>',
          onChange: function () {
             fnInGridListSetting(this.page.selectPage,"&prjGrpId="+pagePrjGrpId,gridObj["sign"],"/chk/chk1000/chk1100/selectChk1100AjaxView.do",false);
          }
      } 
...
var workGrid_config = {
	...
	body: {
		...
		onDBLClick:function(){
			...
			if(usrTyp == "01"){
				...
				var data = {"prjId":this.item.prjId, "reqId": this.item.reqId}; 
				gfnLayerPopupOpen("/req/req4000/req4100/selectReq4105View.do", data, '1470', '900',null,false);
			}
		}
	},
	contextMenu: {
		...
		onClick: function (item, param) {
			...
			if(item.type == "detailPopup"){
				var data = {"mode":"newReq","prjId": param.item.prjId, "reqId": param.item.reqId, "reqProType":param.item.reqProType}; 
				gfnLayerPopupOpen("/req/req4000/req4100/selectReq4104View.do", data, '1470', '900','scroll');
    ...
    page: {
        ...
        lastIcon: '<i class="fa fa-step-forward" aria-hidden="true"></i>',
        onChange: function () {
        	fnInGridListSetting(this.page.selectPage,"&mode=dashboard&workChargerId="+pageUsrId+"&prjGrpId="+pagePrjGrpId,gridObj["work"],"/req/req4000/req4400/selectReq4400ReqWorkListAjax.do",false);
        }
    }
};
...
var usrWorkGrid_config = {
	...
	body: {
		...
		onDBLClick:function(){
			//사용자 작업 목록 팝업 오픈
			var data = {workChargerId: this.item.workChargerId, workChargerNm: this.item.workChargerNm, mode: "dashboard"}; 
	...
	contextMenu: {
		...
		icons: {'arrow': '<i class="fa fa-caret-right"></i>'},
		items: [
			{type: "detailPopup", label: "사용자 작업 목록", icon:"<i class='fa fa-info-circle' aria-hidden='true'></i>"},
			{type: "workChargerDetailPopup", label: "담당자 상세 정보", icon:"<i class='fa fa-info-circle' aria-hidden='true'></i>"}
		],
		popupFilter: function (item, param) {
		...
		onClick: function (item, param) {
			...
			//상세 정보
			if(item.type == "detailPopup"){
				var data = {workChargerId: param.item.workChargerId, workChargerNm: param.item.workChargerNm, mode: "dashboard"}; 
				gfnLayerPopupOpen("/req/req4000/req4100/selectReq4114View.do", data, '1300', '700','scroll');
			}else if(item.type == "workChargerDetailPopup"){
      		if(selItem.workChargerNm == null || selItem.workChargerNm == ""){
      			jAlert('담당자가 없습니다.','알림창');
  ...
  page: {
      ...
      lastIcon: '<i class="fa fa-step-forward" aria-hidden="true"></i>',
      onChange: function () {
      	fnInGridListSetting(this.page.selectPage,"&pageSize=100&prjGrpId="+pagePrjGrpId,gridObj["usrWork"],"/req/req4000/req4400/selectReq4400UsrWorkListAjax.do",false);
      }
  }
};
...
var newReqGrid_config = {
	...
	columns: [
		...
		{key: "prjNm", label: "프로젝트 명", width: 160, align: "center"},
		...
		{key: "reqUsrNum", label: "연락처", width: 120, align: "center"},
		{key: "reqNm", label: "요청 명", width: 392, align: "left"},
		{key: "reqDesc", label: "요청 내용", width: 462, align: "left"}
		],
	body: {
		align: "center",
		columnHeight: 30,
    ...
    page: {
        ...
        lastIcon: '<i class="fa fa-step-forward" aria-hidden="true"></i>',
        onChange: function () {
        	fnInGridListSetting(this.page.selectPage,"&mode=dashboard&reqProType=01&prjGrpId="+pagePrjGrpId,gridObj["request"],"/req/req1000/req1000/selectReq1000ListAjaxView.do",false);
        }
    }
};

//담당 요구사항 그리드 설정 값
var chargerReqGrid_config = {
		showLineNumber: false,
		sortable:true,
		header: {align:"center",columnHeight: 30},
		columns: [
			...
			{key: "reqNm", label: "요구사항 명", width: '28%', align: "left"},
			{key: "reqDesc", label: "요구사항 설명", width: '31%', align: "left"},
			{key: "reqChargerNm", label: "담당자", width: '7%', align: "center"},
			{key: "reqUsrNm", label: "요청자", width: '7%', align: "center"},
			...
			body: {
				...
				onDBLClick:function(){
                	...
                	var prjGrpId = this.item.prjGrpId;
                	 ...
                	 * admReqPage - 전체 요구사항 목록, 요구사항 생성관리(관리자)
                	 */
                	var data = {"mode": "req", "prjId": prjId, "reqId": reqId, "reqPageType": "admReqPage"}; 
                	var popHeight = "800";
					...
					if(reqProType == "01" ){
						...
						if(usrTyp == "01"){
			               	...
			               	gfnLayerPopupOpen("/req/req1000/req1000/selectReq1002View.do", data, '640', '850','scroll');
						}else{
							var data = {"prjId": prjId, "reqId": reqId}; 
                	...
                	// 반려(03)일 경우 req1002.jsp 상세보기 화면으로
						...
						if(popupTargetFlag){
							var data = {"mode":"newReq","prjGrpId": prjGrpId,"prjId": prjId, "reqId": reqId, "reqProType":"02"}; 
							gfnLayerPopupOpen("/req/req4000/req4100/selectReq4104View.do", data, '1470', '900','scroll');
						}else{
							var data = {"prjGrpId": prjGrpId,"prjId": prjId, "reqId": reqId}; 
							gfnLayerPopupOpen("/req/req4000/req4100/selectReq4105View.do", data, '1470', '900','scroll',false);
						}
					}
				}
			},
				...
				items: [
					...
					onClick: function (item, param) {
						...
						//상세 정보
							...
							var reqProType = param.item.reqProType;
							
							//접수대기건인 경우
							if(param.item.reqProType == "01"){
								/*
								 * reqPageType 추가
								 * 요구사항 상세보기(req1002.jsp)에서 항목명 구분을 위해 사용
								 * usrReqPage - 요구사항 요청(사용자) 
								...
								var popupHeight = '845';
								//반려인경우 창 높이 조절
								if(param.item.reqProType == "03"){
									popupHeight = '900';
								}
								
								gfnLayerPopupOpen("/req/req1000/req1000/selectReq1002View.do", data, '640', popupHeight,'scroll');
							}else{
								var data = {"mode":"newReq","prjId":selItem.prjId, "reqId": selItem.reqId, "reqProType": reqProType}; 
								gfnLayerPopupOpen("/req/req4000/req4100/selectReq4104View.do", data, '1470', '900','scroll');
							}
			...
			page: {
				...
				lastIcon: '<i class="fa fa-step-forward" aria-hidden="true"></i>',
				onChange: function () {
					fnInGridListSetting(this.page.selectPage,"&mode=dashboard&prjGrpId="+pagePrjGrpId,gridObj["reqChargerList"],"/req/req4000/req4100/selectReq4100ListAjaxView.do",false);
				}
			}
};
...
var reqGrpGrid_config = {
	...
	contextMenu: {
		...
		onClick: function (item, param) {
			...
			//상세 정보
    			...
    			// 그룹 요구사항 상세보기 팝업 오픈
    			gfnLayerPopupOpen('/req/req3000/req3000/selectReq3003View.do',data,"1200", "830",null);
  ...
  page: {
      ...
      lastIcon: '<i class="fa fa-step-forward" aria-hidden="true"></i>',
      onChange: function () {
    	  fnInGridListSetting(this.page.selectPage,"&mode=prjGrpDashboard&pageSize=100&prjGrpId="+pagePrjGrpId,gridObj["reqGrp"],"/req/req3000/req3000/selectReq3000DshReqGrpListAjax.do",false);
      }
  }
};
	...
	contextMenu: {
		...
		onClick: function (item, param) {
			...
			//상세 정보
				...
				var data = {"mode":"newReq","prjId":selItem.prjId, "reqId": selItem.reqId, "reqProType":"02"}; 
				gfnLayerPopupOpen("/req/req4000/req4100/selectReq4104View.do", data, '1470', '900','scroll');
			}else if(item.type == "regUsrDetailPopup"){
          		if(selItem.reqUsrId == null || selItem.reqUsrId == ""){
        			jAlert('요청자 정보가 없습니다.','알림창');
...
function fnDashboardGridSetting(gridId){
	...
	//담당 요구사항 목록 세팅
	else if(gridId == "reqChargerList"){
		tmp_config = chargerReqGrid_config;
	}
	//작업 목록 세팅
	else if(gridId == "work"){
