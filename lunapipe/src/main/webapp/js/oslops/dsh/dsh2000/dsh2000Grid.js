...
var workGrid_config = {
	...
	columns: [
		...
		{key: "reqNm", label: "요구사항 명", width: 160, align: "center"},
...
var reqDtmOverGrid_config = {
	...
	header: {align:"center",columnHeight: 30},
	columns: [
	    {key: "processNm", label: "프로세스 명", width: 130, align: "center"},
	    {key: "reqNo", label: "요구사항 번호", width: 140, align: "center",formatter:function(){
			return gfnIsNull(this.item.reqNo) ? "-" : this.item.reqNo;
		}},
		{key: "reqNm", label: "요청 제목", width: 250, align: "left"},
		{key: "reqDesc", label: "요청 내용", width: 310, align: "left"},
		{key: "calcReqDay", label: "초과 일수", width: 110, align: "center",formatter:function(){
			return this.item.calcReqDay+"일";
		}},
		{key: "reqChargerNm", label: "담당자", width: 80, align: "center"},
		{key: "regDtm", label: "등록일", width: 140, align: "center",formatter:function(){
			var regDtm = this.item.regDtm;
			// IE에서는 날짜값 뒤에 시간값이 붙어있을경우 format("yyyy-MM-dd")이 정상동작 하지 않아 yyyy-MM-dd만 자른다.
			var regDtmStr = regDtm.substring(0, 10);
 			return new Date(regDtmStr).format("yyyy-MM-dd");
		}},
		{key: "reqStDtm", label: "작업 시작일자", width: 140, align: "center",formatter:function(){
			return gfnIsNull(this.item.reqStDtm) ? "-" : this.item.reqStDtm;
		}},
		{key: "reqEdDtm", label: "작업 종료일자", width: 140, align: "center",formatter:function(){
			return gfnIsNull(this.item.reqEdDtm) ? "-" : this.item.reqEdDtm;
		}},
		{key: "reqStDuDtm", label: "작업 시작 예정일자", width: 140, align: "center",formatter:function(){
			return gfnIsNull(this.item.reqStDuDtm) ? "-" : this.item.reqStDuDtm;
		}},
		{key: "reqEdDuDtm", label: "작업 종료 예정일자", width: 140, align: "center",formatter:function(){
			return gfnIsNull(this.item.reqEdDuDtm) ? "-" : this.item.reqEdDuDtm;
		}}
	],
	body: {
		align: "center",
	...
	contextMenu: {
		...
		popupFilter: function (item, param) {
			...
			// 계획대비 미처리 그리드 외에 나머지 그리드의 contextMenu를 모두 닫는다.
          	fnGridContextMenuClose();
...
var docDtmOverGrid_config = {
	...
	columns: [
		...
		{key: "upDocNm", label: "상위 산출물 명", width: 372, align: "left"},
		{key: "docNm", label: "산출물 명", width: 387, align: "left"},
		{key: "docDesc", label: "산출물 설명", width: 426, align: "left"},
		{key: "docCalcEdDay", label: "초과 일수", width: 120, align: "center",formatter:function(){
...
var flowGrid_config = {
	...
	contextMenu: {
		...
		popupFilter: function (item, param) {
			...
			// 작업흐름 요구사항 그리드 외에 나머지 그리드의 contextMenu를 모두 닫는다.
          	fnGridContextMenuClose();
