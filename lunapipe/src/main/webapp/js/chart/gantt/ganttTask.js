...
function TaskFactory() {
  ...
  this.build = function (id, name, code, level, start, duration, collapsed) {
    // TASK의 start, end 밀리세컨드 값 새로 계산해준다.
...
function Task(id, name, code, level, start, end, duration, collapsed) {
  this.id = id;			// Task ID(요구사항 ID)
  this.name = name;		// Task 명(요구사항 명)
  this.progress = 0; 	// 기본 진척률
  this.progressByWorklog = false;
  this.relevance = 0;
  this.type = "";
  this.typeId = "";
  this.description = "";	// Task 설명
  this.code = code;
  this.level = level;		// Task 레벨
  this.status = "STATUS_UNDEFINED";	// Task 상태
  this.depends = "";		// Task 의존성
  
  this.start = start;		// 시작일 - milisecond 값
  this.duration = duration;	// 시작일 - 종료일 차이
  this.end = end;			// 종료일 - milisecond 값
