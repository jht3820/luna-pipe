...
function GanttMaster() {
  ...
  // 이미지 resource URL 설정
  this.resourceUrl = "/images/gantt/"; //"res/"; 
...
/*
 * 간트 init
 * @param workSpace 간트차트가 생성될 workspace
 */
GanttMaster.prototype.init = function (workSpace) {
  ...
  // Ganttalendar 생성
  // 현재날짜 기준 시간값 세팅, Ganttalendar 생성후 startMillis, endMillis 값을 ganttDrawerSVG.js 
  // 에서 다시 세팅해준다.
  ...
  */
  ...
  // 간트 차트 오른쪽그리드 Task 전체펼침, 전체접기 바인딩
  // 해당부분은 사용시 주석제거
  /* 
  ...
  */
  ...
  //
  // 에디터창 오픈 - TW Gantt Editor을 사용하지 않으므로 해당부분 주석처리
  ...
  */
  ...
  $(window).resize(function () {
   //place.css({width: "100%", height: $(window).height() - place.position().top});  
	place.css({width: "100%", height: "600px"});  
...
function loadI18n(){
  GanttMaster.messages = {
    ...
    "GANTT_FIRST_SEMESTER":"상반기",
    "GANTT_SECOND_SEMESTER":"하반기",
    "GANTT_QUARTER":"분기",
    "GANTT_MONTH":"월",
    "GANTT_DAY":"일",
    "TITLE_STATUS_ACTIVE": "[여유] 완료되지 않았으며 현재일을 기준으로 완료시한이 4일 이상 여유있는 요구사항입니다.",
    "TITLE_STATUS_WAITING":"[임박] 완료되지 않았으며 현재일을 기준으로 완료시한이 3일 이하로 남은 요구사항입니다.",
    "TITLE_STATUS_SUSPENDED":"[초과] 완료되지 않은 요구사항 중 현재일을 기준으로 작업시한을 넘긴 요구사항입니다.",
    "TITLE_STATUS_FAILED": "[실패] 완료시한을 초과하여 완료된 요구사항입니다.",
    "TITLE_STATUS_DONE":"[적기] 완료시한 이내 정상완료된 요구사항입니다.",
  };
}
...
/**
 * 간트차트 프로젝트 데이터 세팅
 * project 에는 Task(요구사항 목록), Resources, Roles가 포함된다.
 * @param project
 */
GanttMaster.prototype.loadProject = function (project) {
  ...
  // 기본 줌레벨 세팅
  // req4600에서 요구사항 조회 후 간트차트에 사용될 데이터 바인딩시 기본 zoom level을 지정한다.
  // 해당 줌레벨 있을경우 그것을 간트차트의 기본 줌레벨로 지정
  if (project.zoom){
...
GanttMaster.prototype.loadTasks = function (tasks, selectedRow) {
  ...
  if (this.tasks && this.tasks.length > 0) {
    ...
    // custom - 조회한 요구사항 목록(task)가 1개일 경우 selectedRow 0으로 처리해야 한다.
    // tasks[] 는 0부터 시작하므로
    if(tasks.length == 1){
    	selectedRow = 0;
    }
  ...
  // Task Load 이후 진척률 input에 이벤트 추가
  $('input[name="progress"]').blur(function(event){

	// 현재 선택된 요구사항(Task) 정보
	var currentTask = ge.currentTask;  	  
	// 선택한 요구사항의 진척률 값을 가져온다.
	var progressVal = $(this).val();
	// 숫자 범위 체크  
	var rtnProgressChk = fnValidPercentage(progressVal); 

	// 입력한 진척률 값이 숫자 범위 0~100이 아닐경우
	if(!rtnProgressChk){
		// 현재 Task의 진척률 값을 세팅
		progressVal = currentTask.progress;
	}
...
/*
 * 간트차트 Task를 가져온다
 * @param taskId 간트차트 task ID - 현재 task id는 요구사항 id 
 */
...
GanttMaster.prototype.fullScreen = function () { 
	...
	// AxSearch, 간트 차트 영역 Div 사이즈 조절
	$("#gantt_main_frame").toggleClass("ganttFullScreen").resize();
	
	// 간트차트 영역 div에 ganttFullArea 클래스 추가
	$("#TWGanttArea").toggleClass("ganttFullArea");
	
	// 기본 간트차트 높이 600px
	var ganttAreaHeight = "600";
	
	// 풀스크린 전 aside의 최소높이값을 가져온다. 
	// 간트차트 풀스크린 후 다시 원래사이즈 화면으로 되돌릴때 가져온 aisde의 최소높이값 사용
	var beforeMinHeight = $('aside').css('min-height');
	
	// 풀스크린 버튼
	var $fullScreenBtnObj = $(".top_searchGroup").find(".fullScreenBtn");
	
	// 간트차트 풀 스크린일 경우
	if( $("#TWGanttArea").hasClass("ganttFullArea")){
		
		// 화면 높이와 AXSearch div의 높이를 가져온다.
		var winHeight = $(window).height();
		var searchHeight = $("#AXSearchTarget").outerHeight();

		// 두 높이의 차이를 구한다.
		ganttAreaHeight = winHeight - searchHeight;

		// 풀스크린일 경우 풀스크린 버튼의 아이콘 변경
		$fullScreenBtnObj.find("i").removeClass("fa-expand");
		$fullScreenBtnObj.find("i").addClass("fa-compress");
		
		// 전체화면일 경우 body overflow 추가 (화면 우측 스크롤 제거를 위해 사용)
		$("body").css("overflow-y","hidden");
		
	}else{
		// section, aside의 최소 높이값을 풀스크린 전 최소높이 값으로 되돌린다.
		$('section').css('min-height', beforeMinHeight);
		$('aside').css('min-height', beforeMinHeight);
		
		// 풀스크린이 아닐경우 경우 풀스크린 버튼의 아이콘 변경
		$fullScreenBtnObj.find("i").removeClass("fa-compress");
		$fullScreenBtnObj.find("i").addClass("fa-expand");
		
		// 인라인 스타일 overflow-y 제거
		$("body").attr('style','');
	}
	
	// 간트차트 영역에 높이 세팅
	$("#TWGanttArea").css({height: ganttAreaHeight});  
