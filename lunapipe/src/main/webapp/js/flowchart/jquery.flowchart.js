...
// "colorize" the widget name
    $.widget("flowchart.flowchart", {
        ...
        _getOperatorFullElement: function (operatorData) {
            ...
            var flowContentBgColor = operatorData.properties.flowContentBgColor;
            ...
            var flowSignAfter = operatorData.properties.flowSignAfter;
            ...
            var flowDpl = operatorData.properties.flowDpl;
            var flowAuth = operatorData.properties.flowAuth;
            var flowReqLink = operatorData.properties.flowReqLink;
            var flowReqLinkTest = operatorData.properties.flowReqLinkTest;
            var flowDocLink = operatorData.properties.flowDocLink;
            var flowOptCnt = operatorData.properties.flowOptCnt;
          //  var flowNextId = operatorData.properties.flowNextId;
            
            var $operator = $('<div class="flowchart-operator" style="background-color:'+flowContentBgColor+';color:'+flowContentColor+';border-width:1px;border-style:solid;border-color:'+flowTitleBgColor+'"></div>');
            $operator.addClass(infos.class);

            var flowIcon = "";
/*            //flowNextId 없는경우
            if(gfnIsNull(flowNextId) || flowNextId == "null"){
            ...
            //작업
            if(flowWork == "on"){
            	flowIcon += "<li class='fa fa-code-branch' title='작업'></li>";
            }
            //리비전
            if(flowRevision == "on"){
            	flowIcon += "<li class='fa fa-code' title='리비전 저장유무'></li>";
            }
            //배포계획 체크
			if(flowDpl == "on"){
			...
			//요구사항 연결
			if(flowReqLink == "on"){
				flowIcon += '<li class="fas fa-link" title="요구사항 연결 유무"></li>';
			}
			//연결 요구사항 테스트
			if(flowReqLinkTest == "on"){
				flowIcon += '<li class="fas fa-edit" title="연결 요구사항 테스트 유무"></li>';
			}
			//산출물 연결
			if(flowDocLink == "on"){
				flowIcon += '<li class="fas fa-copy" title="요구사항 산출물 연결 유무"></li>';
			}
            //추가 항목
            ...
            flowIcon = "<div class='flowOptionDiv' style='border-color:"+flowTitleBgColor+";border-bottom: 1px dashed #fff;'>"+flowIcon+"</div>";
            
            var $operator_title = $('<div class="flowchart-operator-title" style="background-color:'+flowTitleBgColor+';color:'+flowTitleColor+';"></div>');
            
            $operator_title.html(flowIcon+'<div class="flow-title_group">'+infos.title+'</div>');
        ...
        /** 2018-08-27 operator, title css변경 추가 **/
        editStyleOperator: function (operatorId, type, css) {
        	var $objTarget = null;
        	
        	//제목
        	if(type == "title"){
        		$objTarget = $(this.data.operators[operatorId].internal.els.title);
        	}
        	//내용
        	else if(type == "operator"){
        		$objTarget = $(this.data.operators[operatorId].internal.els.operator);
        	}
