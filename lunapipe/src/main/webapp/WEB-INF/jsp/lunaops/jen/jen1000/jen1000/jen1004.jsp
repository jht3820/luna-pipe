			...
			<div class="jen1004_bottom_bld_content" id="jen1004_bottom_bld_content">
				<c:if test="${not empty jobBldNumList}">
					<c:forEach items="${jobBldNumList}" var="map">
						<div class="jen1004_middle_row" dplid="${map.dplId}" jenid="${map.jenId}" jobid="${map.jobId}" bldseq="${map.bldSeq}" onclick="fnPopJobConsoleLogLoad(this)">
							<div class="jen1004_middle_cell">${map.bldNum}</div>
							<div class="jen1004_middle_cell">${map.bldResult}</div>
							...
							<div class="jen1004_middle_cell">${map.regUsrNm}</div>
						</div>
					</c:forEach>
				</c:if>
			</div>
